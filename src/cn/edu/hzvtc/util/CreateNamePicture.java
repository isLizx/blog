...
/*    public static void main(String[] args) throws IOException {
        String name = "xiaozi";
        File file = new File("C:/Users/李智翔/Desktop");
        if(file.exists()){
        	File file1 = new File(file, "/新建文件夹");
        ...
        String filename = generateImg(name, "C:/Users/李智翔/Desktop/新建文件夹");
        System.out.println(filename);
    }*/
	

    public String createHead(String name,String realPath){
    	// 文件名
    	String fileName = isFileImageName(name);
    	// 路径文件名
    	File fileImage = new File(realPath + "/" + fileName + ".jpg");
    	// 判断文件是否已经存在
    	if(!fileImage.exists()){
    		try {
				generateImg(name, realPath);
			} catch (IOException e) {
				e.printStackTrace();
			}
    	}
		return "../upload/" + fileName + ".jpg";
    }

    // 根据name判断image名
    public String isFileImageName(String name){
     ...
     * @throws IOException
     */
    public void generateImg(String name, String outputPath)
            throws IOException {
        int width = 100;
        int height = 100;
        String nameWritten;
        ...
        nameWritten = isFileImageName(name);

        String filename = outputPath + File.separator + nameWritten + ".jpg";
